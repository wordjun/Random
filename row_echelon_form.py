...
def row_echelon_form(matrix, row, column):
    ...
    for k in range(0, row-1):
        for i in range(k, row-1):
            ...
            first_num = matrix[k][temp] #assign the first number on current row, first column.
            if matrix[i+1][temp] != 0:  #if the second number is not zero,
                second_num = matrix[i+1][temp]  #assign it as the second one.
            else:   #if it is a zero,
